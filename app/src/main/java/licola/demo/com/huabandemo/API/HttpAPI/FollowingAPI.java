...
public interface FollowingAPI {
    ...
    @GET("following")
    Observable<AttentionPinsBean> httpsMyFollowingPinsRx(@Header(Constant.Authorization) String authorization, @Query("limit") int limit);

    //https://api.huaban.com/following?limit=40&max=670619456
    //我的关注图片的 后续滑动联网
    @GET("following")
    Observable<AttentionPinsBean> httpsMyFollowingPinsMaxRx(@Header(Constant.Authorization) String authorization, @Query("max") int max, @Query("limit") int limit);

    //https://api.huaban.com/boards/following?page=1&per_page=20
    //我的关注画板
    @GET("boards/following")
    Observable<AttentionBoardListBean> httpsMyFollowingBoardRx(@Header(Constant.Authorization) String authorization, @Query("page") int page, @Query("per_page") int per_page);
